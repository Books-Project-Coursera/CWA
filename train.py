    ...
    def on_model_save(self, trainer):
        ...
        if not ckpt.exists():
            raise FileNotFoundError(
                ...
                "Strategy 2 cần save=True và save_period=1."
            )
...
def build_train_args(data_yaml):
    ...
    if Config.any_method():
        ...
        # Không method nào → không cần epoch checkpoint, nhưng VẪN phải
    ...
    # EXTRA_TRAIN_ARGS phá invariant này.
    if Config.any_method():
        train_args["val"] = True
        train_args["save"] = True
        train_args["save_period"] = 1
...
def _train_one_seed(seed, exp_dir):
    ...
    if Config.any_method():
        # Raw-weight validation là NỀN CHUNG của cả ba method: fitness val',
        # best.pt và early stopping đều tính trên raw weights, nên trajectory
        # và tín hiệu chọn checkpoint giống hệt nhau ở mọi tổ hợp --method.
        # Khi không bật top-k vẫn giữ 1 checkpoint để Strategy 1 là raw FP32
        # (cùng precision với các bản average) thay vì best.pt FP16.
        ...
        print(
            f"  Checkpoint pool: giữ Top-{keep} RAW checkpoint theo raw-model "
            "fitness validation cao nhất"
        )

    if Config.method_enabled("ema") or Config.method_enabled("swa"):
        # EMA/SWA chỉ QUAN SÁT weights (shadow ngoài training loop) — không
        # optimizer step nào, không gradient nào của chúng đi vào model.
...
def train_detector():
    ...
    2. Với MỖI seed: tách val' độc lập từ train → train → đánh giá Strategy 1 vs
       Strategy 2 (Top-K average) trên split test → Excel + charts của seed đó
    ...
    config_snapshot = {
        ...
        "strategy1_source": "rank#1 raw FP32" if Config.STRATEGY1_FROM_RAW_TOPK else "best.pt (FP16)",
        ...
        "checkpoint_weight_source": "raw" if Config.USE_STRATEGY2 else "ultralytics_default",
