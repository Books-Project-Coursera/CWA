...
def strategy_avg_name(k):
    return f"Strategy 2 (Top-{int(k)} avg)"
...
def update_bn_stats(weights_path, data_yaml, num_batches=None, device=None, mosaic_closed=None):
    ...
    Strategy2_TinyImageNet, thích ứng cho YOLO.
...
def run_strategy_evaluation(run_dir, data=None, split=None, seed=None, excel_path=None):
    """
    Đánh giá các strategy trên split báo cáo (mặc định Config.EVAL_SPLIT='test'):

      - Strategy 1 (best.pt)      : checkpoint fitness cao nhất, FP16 do
    ...
    if best_weights.exists():
        results[STRATEGY_BASELINE] = _val(
            best_weights, STRATEGY_BASELINE, f"Strategy 1 — best.pt (split={split})"
        )
    else:
        ...
        if not ranked:
            ...
            for k in sorted({int(k) for k in Config.TOP_K_VALUES}):
                ...
                try:
                    ...
                    results[name] = _val(
                        avg_path, name, f"Strategy 2 — Top-{k} average (split={split})"
                    )
                finally:
    ...
    print("=" * 70)
    baseline_metrics = results.get(STRATEGY_BASELINE)
    baseline_value = (
    ...
    for name, metrics in results.items():
        ...
        delta = ""
        if baseline_value is not None and name != STRATEGY_BASELINE:
...
def export_to_excel(run_dir, strategy_results=None, data=None, split=None,
    ...
    - "Summary"    : run info + overall metrics theo strategy (kèm Δ so với
                     Strategy 1) + per-class AP theo strategy.
    ...
    if strategy_results:
        ...
        # Δ so với Strategy 1 để đọc nhanh strategy nào thắng
        baseline = next(
            (r for r in overall_rows if r["Strategy"] == STRATEGY_BASELINE), None
        ) or overall_rows[0]
...
def _delta_frame(summary_df, metric_columns, baseline_name):
    ...
    base = summary_df[summary_df["Strategy"] == baseline_name].set_index("Seed")
    rows = []
    for name in dict.fromkeys(summary_df["Strategy"]):
        if name == baseline_name:
            continue
        group = summary_df[summary_df["Strategy"] == name].set_index("Seed")
        row = {"Strategy": name, "Baseline": baseline_name, "N seeds": int(len(group))}
        for column in metric_columns:
...
def export_multi_seed_summary(all_runs_results, output_dir, exp_name=None, extra_info=None):
    ...
    Sheets (theo thứ tự mở trong Excel):
      ...
      2. "DeltaVsBaseline"   — Δ ghép cặp theo seed so với Strategy 1 + tỉ lệ thắng.
    ...
    for run in all_runs_results:
        ...
        for strat_name, metrics in (run.get("strategy_results") or {}).items():
            ...
            for row in extract_per_class_metrics(metrics):
                per_class_rows.append({"Seed": seed_val, "Strategy": strat_name, **row})
    ...
    mean_std_df = _mean_std_frame(summary_df, ["Strategy"], metric_columns)

    baseline_name = (
        STRATEGY_BASELINE if STRATEGY_BASELINE in set(summary_df["Strategy"])
        else summary_df["Strategy"].iloc[0]
    )
