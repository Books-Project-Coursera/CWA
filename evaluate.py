"""
Evaluation script with 2 strategies and result export
"""
import os
import copy
import torch
...
def strategy_2_top_k_average(model_name, checkpoint_manager, test_loader, train_loader, num_classes, device, class_names=None, save_dir=None):
    """
    Strategy 2: Average top-K checkpoints from the candidate pool and evaluate

    The pool only holds epochs that set a new best-so-far validation loss, up to
    the epoch where the pool patience was exhausted. Epochs that never broke the
    record - and everything after the cutoff - are excluded by construction.
    ...
    print(f"\n  Strategy 2: Top-K checkpoint averaging (from candidate pool)")
    print(
        f"    Candidate pool ({len(checkpoint_manager.candidate_pool)} checkpoints): "
        + ", ".join(
            f"epoch {epoch} ({val_loss:.4f})"
    ...
    for k in Config.TOP_K_VALUES:
        ...
        # Get top K checkpoints from the Strategy 2 candidate pool
        top_k = checkpoint_manager.get_top_k_from_pool(k)

        if len(top_k) < k:
            print(f"      Warning: Only {len(top_k)} checkpoints available")

        print(f"      Averaging epochs: {[ep for ep, _, _ in top_k]}")
...
def evaluate_all_strategies(model_name, checkpoint_manager, test_loader, train_loader, num_classes, device, class_names=None, save_dir=None):
    """
    Evaluate all strategies for a model
