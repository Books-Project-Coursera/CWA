...
main.py  → Config.validate_config()  → train.train_detector()
                                          ...
                                          │      B. EVAL    (Strategy 1 / Top-1 / Top-K avg)
...
`strategy2_checkpoints.json` (bảng xếp hạng).

## B. Đánh giá trên `test` (`run_strategy_evaluation`)

`rank_checkpoints()` đọc JSON → list `(file, fitness, epoch)` giảm dần.

| # | Strategy | Weights | BN |
|---|---|---|---|
| 1 | `Strategy 1 (best.pt)` | best.pt (FP16) | nguyên |
| 2 | `Top-1 (best raw ckpt)` | `ranked[0]` raw FP32, **không average** | nguyên |
| 3 | `Strategy 2 (Top-2 avg)` | average 2 ckpt đầu | **recalibrate** |
| 4 | `Strategy 2 (Top-3 avg)` | average 3 ckpt đầu | **recalibrate** |
| 5 | `Strategy 2 (Top-4 avg)` | average 4 ckpt đầu | **recalibrate** |
| 6 | `Strategy 2 (Top-5 avg)` | average 5 ckpt đầu | **recalibrate** |
...
- `DeltaVsBaseline` — Δ ghép cặp theo seed vs Strategy 1 + số seed thắng
