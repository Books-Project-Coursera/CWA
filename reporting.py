"""
Reporting layer cho Strategy 2 - Object Detection (Ultralytics YOLO + Pascal VOC).
...
Cây thư mục kết quả một experiment (`python main.py train --exp-name <NAME>`):
    ...
    └── seeds/
        ...
        │       └── test_<strategy>/   # PR/F1 curve + confusion matrix trên test
        └── seed_10/ ...
...
def slugify(text):
    """Tên file/thư mục an toàn từ tên strategy ('Strategy 2 (Top-3)' → 'strategy_2_top_3')."""
...
def paired_t_test(deltas):
    ...
    Dùng cho so sánh Strategy 2 vs baseline: cùng một seed → cùng data split và
...
def _run_info_rows(run_dir, data, split, seed, summary_source):
    ...
    return [
        ...
        ("strategy2 top_k_values", str(Config.TOP_K_VALUES) if Config.USE_STRATEGY2 else "OFF"),
...
def _overall_dataframe(records):
    ...
    if df.empty:
        return df
    lead = ["Strategy", "K", "BN recal", "Source epochs"]
...
def _per_class_dataframe(records):
    ...
    for record in records:
        for row in extract_per_class_metrics(record["metrics"]):
            rows.append({"Strategy": record["name"], **row})
...
def _build_summary_frames(all_runs_results):
    ...
    for run in all_runs_results:
        ...
        for record in records:
            ...
            for row in extract_per_class_metrics(record["metrics"]):
                per_class_rows.append({"Seed": seed, "Strategy": name, **row})
...
def _mean_std_frame(per_seed_df, order, shorts):
    ...
    rows = []
    for name in order:
        group = per_seed_df[per_seed_df["Strategy"] == name]
        if group.empty:
            continue
        row = {
            "Strategy": name,
...
def _delta_frame(per_seed_df, order, baseline_name):
    ...
    baseline = per_seed_df[per_seed_df["Strategy"] == baseline_name].set_index("Seed")
    rows = []
    for name in order:
        if name == baseline_name:
            continue
        group = per_seed_df[per_seed_df["Strategy"] == name].set_index("Seed")
        shared = [s for s in group.index if s in baseline.index]
        if not shared:
            continue
        row = {"Strategy": name, "Baseline": baseline_name, "N paired seeds": len(shared)}
        for col in metric_cols:
...
def _per_class_mean_std_frame(per_class_df, order):
    ...
    rows = []
    for name in order:
        group = per_class_df[per_class_df["Strategy"] == name]
        if group.empty:
            continue
        for class_name, class_group in group.groupby("Class", sort=False):
            ...
            rows.append(row)
    df = pd.DataFrame(rows)
    return df.sort_values(["Class ID", "Strategy"]) if not df.empty else df
...
def _chart_topk_curve(plt, mean_std_df, baseline_name, metric, out_path):
    ...
    """
    topk = mean_std_df[mean_std_df["K"].notna() & (mean_std_df["Strategy"] != baseline_name)]
    ...
    base_row = mean_std_df[mean_std_df["Strategy"] == baseline_name]
...
def _chart_strategy_dots(plt, mean_std_df, baseline_name, metric, out_path):
    ...
    topk_mask = [
        ...
        for k, name in zip(mean_std_df["K"], mean_std_df["Strategy"])
    ]
...
def _chart_per_seed_paired(plt, per_seed_df, order, shorts, metric, out_path):
    ...
    means = []
    for name in order:
        values = [float(v) for v in per_seed_df[per_seed_df["Strategy"] == name][metric].tolist()]
...
def export_experiment_charts(mean_std_df, per_seed_df, order, shorts, baseline_name, charts_dir):
    ...
    _try(_chart_strategy_dots, plt, mean_std_df, baseline_name, PRIMARY_METRIC,
         charts_dir / "02_strategy_mAP50-95.png")
    _try(_chart_strategy_dots, plt, mean_std_df, baseline_name, "mAP@0.5",
         charts_dir / "03_strategy_mAP50.png")
...
def print_multi_seed_table(all_runs_results):
    ...
    header = f"  {'Strategy':<{label_width}}" + "".join(f" | seed {s:<7}" for s in seeds)
    header += " | mean ± std"
    print(header)
    print("  " + "-" * (len(header) - 2))
