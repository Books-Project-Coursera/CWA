...
checkpoints are ranked on, and it is always disjoint from the split the
...
\begin{table*}[t]
  ...
  \caption{Datasets and evaluation protocol. ``Selection'' is the split the
    checkpoints are ranked on; it is disjoint from ``Final
...
vehicle parts with 23 classes and a 3,156/401/276 split; as above, the
selection split is held out from the training pool and the test split is used
only for reporting.
...
$\{1,10,42,100,500\}$. For a given seed, the baseline and all $k$ come from the
\emph{same} completed run and the same checkpoint set, so they share
architecture, initialization, data order, optimizer, schedule and duration, and
differ only in the post-training selection rule; any difference between them is
attributable to that rule and not to training variance. We report mean $\pm$
standard deviation over the five seeds. All values of $k$ are reported as a
sensitivity analysis; $k$ is never chosen on the final evaluation split.
