...
The rule's leverage comes from disagreement among near-equal checkpoints, so it
should help most when late-epoch validation is noisy and least when the
criterion has a sharp, stable optimum. Our data agree: the smallest gains occur
on Burmese Grape, whose baselines sit closest to their ceiling. When a run
early-stops after very few epochs there are barely $k$ meaningful candidates
and the method degenerates gracefully towards the baseline, as the ViT-Base
rows show.
