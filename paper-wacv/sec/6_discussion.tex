...
\Cref{sec:related} argued that ranking by score differs from ranking by
recency. That is a claim about which checkpoints get picked, not about
accuracy, and it is directly checkable. We looked at the epochs actually
selected in the CIFAR-100 runs, where the per-seed records are available: six
architectures $\times$ five seeds $=30$ runs, so 150 selected checkpoints at
$k{=}5$. \Cref{tab:selection} summarizes where they sit.

\begin{table}[t]
  \centering \small
  \setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.12}
  \caption{Where the selected checkpoints sit on CIFAR-100, averaged over five
    seeds. \emph{Epochs run} is how many epochs actually completed, since early
    stopping usually ends a run before its budget, and \emph{best epoch} is the
    one the baseline keeps. \emph{Span} is the distance from the earliest to
    the latest of the five selected epochs, so a block of consecutive epochs
    would span exactly 5. The last column counts how many of the five fall in
  the last five epochs of the run.}
  \label{tab:selection}
  \resizebox{\linewidth}{!}{%
\begin{tabular}{@{}lccccc@{}}
    \toprule
    Model & \makecell{Epochs\\run} & \makecell{Best\\epoch} & Span
    & \makecell{Not consec.\\(of 5 seeds)}
    & \makecell{In last\\5 epochs} \\
    \midrule
...
and in 20 of the 30 runs none of the five does. The earliest selected
checkpoint comes 16.4 epochs before the end of training on average, and even
the baseline's own best epoch sits at about 66\% of the run---a direct
consequence of early stopping with patience 8--12. Averaging the last five
checkpoints of these same runs would therefore combine a largely different set
of weights.
...
\paragraph{Storage.}
Keeping every epoch means 41.3 checkpoints per run on average. As noted in
\cref{sec:method}, keeping only a running top-$k$ set gives an identical
result, so the practical cost is six checkpoints at $k{=}5$.
...
\paragraph{When it should help.}
The rule gains from disagreement among near-equal checkpoints, so it should
help most when late-epoch validation is noisy and least when the validation
score has a sharp, stable optimum. Our data agree: the smallest gains are on
Burmese Grape, whose baselines sit closest to their ceiling. When a run
early-stops after very few epochs there are barely $k$ meaningful candidates
and the method falls back gracefully towards the baseline, as the ViT-Base rows
in \cref{tab:selection} show.
