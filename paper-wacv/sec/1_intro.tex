...
All the rule needs from the task is a way to order the checkpoints, which is
what makes it portable. Classification pipelines rank by validation loss;
detection and segmentation pipelines already rank by a validation fitness
score. We use whichever one the pipeline provides, and only the sort direction
changes, which lets us run the same rule on three tasks with no task-specific
tuning.
...
\begin{enumerate}
\itemsep0em
\item A post-training top-$k$ averaging rule that needs no change to the
training pipeline, no secondary optimization and no extra inference cost, and
that is stated without reference to any particular task (\cref{sec:method}).
