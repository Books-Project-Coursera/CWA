...
Our rule differs in when it acts and in what it needs: it runs after training
has terminated, prescribes no optimizer, schedule, cycle structure or stopping
rule, performs no secondary optimization, and requires only the epoch
checkpoints and validation scores standard pipelines already produce.
...
cut the \emph{training} cost of obtaining diverse members
\cite{huang2017snapshotensemblestrain1}, but the inference overhead remains,
and our setting assumes it is unavailable.
